...
class SheepDogEnv(gym.Env):
    ...
    def _get_obs(self):
        return {
            ...
            "sheep_v": self.sheep_v,
            "dog_v": self.dog_theta_v*self.circle_R,
    ...
    def _get_reward(self, _info, info, _ob, ob):
        return -(
            # 与上一步相比，羊与圆圈的距离减少了多少
            ((_ob["circle_R"] - _ob["sheep_polar_coor_r"]) - \
             (ob["circle_R"] - ob["sheep_polar_coor_r"]))
            # 与上一步相比，羊与犬的距离增大或减少了多少
            + (np.max(info["distance"])-np.max(_info["distance"]))
        )+self.sheep_v*self.dt
    ...
    def step(self, action: Any) -> tuple[Any, SupportsFloat, bool, bool, dict[str, Any]]:
        ...
        _observation_space = self._get_obs()
